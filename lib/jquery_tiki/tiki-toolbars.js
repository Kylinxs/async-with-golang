...
/* new for 25.1 toolbar-dialogs revamp */


function toolbarDialog(dialog, domId) {
    debugger;
}

// previous toolbars pre 25.1
...
function displayDialog( ignored, list, area_id, clickedElement ) {
...
function editorSettings(domId) {
    const $editorSettingsModal = $('body > .modal.fade:not(.show):first');

    const $textarea = $("#" + domId),
        $form = $textarea.parents("form");

    $editorSettingsModal.one("show.bs.modal", event => {
        const $modal = $(event.target);

        $(".modal-body", $modal).html($("#editor-settings").html());
        $(".modal-title", $modal).text(tr("Editor Settings"));
    });

    $editorSettingsModal.one("shown.bs.modal", event => {
        const $editorSelect = $("#editor-select", event.target),
            $syntaxSelect = $("#syntax-select", event.target),
        ...
        $(".btn-primary", $editorSettingsModal).off("click").click(function () {
    ...
    $editorSettingsModal.modal("show");
}
